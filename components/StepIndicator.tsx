          ...
          return (
            <React.Fragment key={label}>
              <div className="flex flex-col items-center">
                ...
                <p className={`mt-2 text-sm font-medium ${isActive ? 'text-cyan-400' : 'text-gray-500'}`}>
                  {label}
                </p>
              </div>
              {index < distinctSteps.length - 1 && (
                <div className={`flex-1 h-1 mx-2 transition-all duration-500 ${isActive && index < activeIndex ? 'bg-cyan-500' : 'bg-gray-700'}`}></div>
              )}
            </React.Fragment>
          );
        })}
