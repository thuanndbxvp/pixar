...
export const getStep3Prompt = (expandedStory: string, aspectRatio: '9:16' | '16:9', visualStyle: VisualStyle, selectedCharacter: LibraryCharacter | null): string => {
    let characterTaskInstruction: string;
    ...
    if (selectedCharacter) {
        ...
        characterTaskInstruction = `
**TASK 1: ADAPT STORY & LOCK CHARACTER**
The user has selected a specific character. Your primary task is to adapt the expanded story to naturally and logically feature this character.

**CRITICAL ADAPTATION RULES:**
1.  **Prioritize Plausibility:** The character's actions in the script MUST be believable for their species (${selectedCharacter.species}). If the original story contains actions unsuitable for this species (e.g., a pig flying), you must creatively REWRITE those scenes. Find an alternative, logical way for the character to achieve a similar outcome while respecting their physical nature (e.g., the pig builds a jetpack, uses a catapult, etc.).
2.  **Preserve the Core:** While adapting actions, you must maintain the original story's emotional arc, core conflict, and final twist. The "how" of the action can change, but the "what" and "why" of the story must remain intact.
3.  **Use Exact Definition:** You MUST use the following character definition exactly as provided. Do not alter it. Present this character under a "CHARACTERS" heading before the script. This cast is now LOCKED.
...
---
        ...
        characterTaskInstruction = `
**TASK 1: ANALYZE & DEFINE THE CHARACTER(S)**
1.  Carefully read the expanded story to identify the main character(s).
2.  Determine their species (e.g., dog, elephant, chicken, robot etc.) based on the narrative.
3.  For each character, create a detailed description using the provided CHARACTER SCHEMA. The 'Species' field MUST be filled in based on your analysis of the story.
4.  The 'Visual Style Keywords' MUST align with the overall visual style selected by the user.
5.  Present the final cast first under a "CHARACTERS" heading. This cast is now LOCKED and must be used consistently.
        `;
    }

    return `
WORKFLOW STEP 3 — ADAPT STORY & CREATE SCRIPT

**USER-DIRECTED CONTEXT (CRITICAL):**
The AI must adhere strictly to the user's directorial choices. These choices are non-negotiable and must shape the entire script.
- **VISUAL STYLE:** ${visualStyle.description}
- **ASPECT RATIO:** ${aspectRatio} (${aspectRatio === '9:16' ? 'vertical composition' : 'horizontal composition'})
- **CHARACTER:** ${selectedCharacter ? `Pre-defined as ${selectedCharacter.name}, a ${selectedCharacter.species}` : 'To be generated by AI based on the story.'}
...
1.  After defining/locking the character(s), break down the (potentially adapted) story into 10–15 cinematic scenes.
2.  **IMPORTANT**: All scenes must be framed and described with the **${aspectRatio} aspect ratio** in mind. Emphasize ${aspectRatio === '9:16' ? 'verticality, height, and tight shots' : 'width, landscapes, and horizontal movement'}.
3.  The atmosphere, lighting, and textures described in every scene MUST reflect the chosen **Visual Style**.
4.  For each scene, provide:
    - Setting: place, time of day, lighting, atmosphere (consistent with the Visual Style).
    - Characters: repeat the full, locked description for any character who appears.
    - Action: blocking, micro-movements, interactions. These actions MUST be plausible for the locked character(s).
